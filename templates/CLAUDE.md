# CLAUDE.md: Red Team Operating Model

Operating context for a project that runs the red team skills with Claude Code. Copy this file to the root of the engagement project (alongside the installed `rt-*` skills). It keeps the guardrails and the pipeline in context every session, before any skill is invoked.

This is an AI-assisted, **human-led** red team operating model built on a threat-informed, intelligence-led methodology. The work is capability-building and control-validation, not independent assurance. The work is done by composable skills invoked as slash commands (`/rt-govern`, `/rt-intel`, ...), handing off over one engagement artifact. The agent assists; the human authorizes, decides, and executes.

## The pipeline

Six phases, beginning at intake, each invoked as a slash command:
